...
: CStructuredModel(features, labels)
{
	...
	{
		v  = mf->get_feature_vector(i);
		N += v.num_cols;
	}

	// Choose the supporting points so that the same number of points
	// fall on each bin
	SGVector< float64_t > a = CMath::linspace(1, N, m_num_plif_nodes+1);
	...
	for ( int32_t f = 0 ; f < D ; ++f )
	{
		...
		{
			v = mf->get_feature_vector(i);
			for ( int32_t j = 0 ; j < v.num_cols ; ++j )
				signal[idx++] = v(f,j);
		}
		...
		for ( int32_t s = 0 ; s < S ; ++s )
		{
			...
			m_plif_matrix->set_element(plif, s, f);
		}
	}
